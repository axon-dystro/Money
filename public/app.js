...
function expensesForBucket(b) {
  return monthExpenses().filter(e => e.bucketId === b.id || (!e.bucketId && b.id === freeBucketId()));
}
...
function totals() {
  ...
  const cancel = sum(data.cancelableCosts || []);
...
function renderCalendar() {
  ...
  for (let d = 1; d <= days; d++) {
    ...
    html += `<button class="day ${entries.length ? 'hit' : ''}" type="button" onclick="showDay('${date}')"><b>${d}</b>${entries.slice(0, 3).map(e => `<span>${catAbbr(bucketName(e.bucketId))} ${euro(e.amount)}</span>`).join('')}${entries.length > 3 ? `<em>+${entries.length - 3}</em>` : ''}</button>`;
  }
...
function renderPrintReport() {
  ...
  const expenseRows = [...monthExpenses()].sort((a, b) => parseDateInput(a.date).localeCompare(parseDateInput(b.date))).map(e => `<tr><td>${formatDateInput(e.date)}</td><td>${esc(bucketName(e.bucketId))}</td><td>${esc(e.note || '')}</td><td class="num">${euro(e.amount)}</td></tr>`).join('') || '<tr><td colspan="4" class="print-muted">Keine Ausgaben.</td></tr>';
...
function printCalendarHtml() {
  ...
  for (let d = 1; d <= days; d++) {
    const date = `${y}-${String(m + 1).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
    const entries = monthExpenses().filter(e => parseDateInput(e.date) === date);
    html += `<div class="print-day"><b>${d}</b>${entries.slice(0, 2).map(e => `<span>${catAbbr(bucketName(e.bucketId))} ${euro(e.amount)}</span>`).join('')}${entries.length > 2 ? `<span>+${entries.length - 2} weitere</span>` : ''}</div>`;
  }
  html += '</div>';
  return html;
}
...
function csv() {
  ...
  for (const e of monthExpenses()) rows.push([formatDateInput(e.date), bucketName(e.bucketId), e.note || '', e.amount]);
...
function wire() {
  ...
  q('#bucketForm').onsubmit = e => {
    e.preventDefault();
    api('/api/bucket', { method: 'POST', body: JSON.stringify({ name: q('#bucketName').value, mode: q('#bucketMode').value, amount: num(q('#bucketAmount').value), unitAmount: num(q('#bucketUnitAmount').value), unitCount: num(q('#bucketUnitCount').value), periods: num(q('#bucketPeriods').value) || 4 }) });
    ['#bucketName', '#bucketAmount', '#bucketUnitAmount', '#bucketUnitCount', '#bucketPeriods'].forEach(s => q(s).value = '');
  };
